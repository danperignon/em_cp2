 ...
 */
export function identifyProblemPatterns(problem: ProblemDefinition): ProblemPattern[] {
  ...
  const matches = PROBLEM_PATTERNS.map(pattern => {
    ...
    const indicatorMatches = pattern.indicators.filter(indicator => 
      words.some(word => word.includes(indicator) || indicator.includes(word))
    ).length;
 ...
 */
function calculateContextScore(problem: ProblemDefinition, pattern: ProblemPattern): number {
  let score = 0;
  ...
  if (problem.domain) {
    ...
    const relevantTypes = domainKeywords[problem.domain as keyof typeof domainKeywords] || [];
 ...
 */
function calculateComplexityScore(problem: ProblemDefinition, pattern: ProblemPattern): number {
  const complexityMap = {
    'simple': 'low',
    'moderate': 'medium', 
    'complex': 'high',
 ...
 */
export function selectDecompositionStrategy(
 ...
 */
export function estimateSolvingParameters(patterns: ProblemPattern[]) {
  if (patterns.length === 0) {
    return {
      estimatedSteps: 5,
