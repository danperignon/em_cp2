...
class SequentialThinkingServer extends MCPServer {
  private logger: Logger = new Logger('sequential-thinking');
  ...
  protected async setupHandlers(): Promise<void> {
      ...
      try {
        switch (name) {
          case 'decompose_problem':
            const result = await this.toolHandlers.handleDecomposeProblem(args as any);
            if (result.success) {
              return {
                content: [
                  {
          ...
          case 'save_session':
            const saveResult = await this.toolHandlers.handleSaveSession(args as any);
            return {
              content: [
                {
                  type: 'text',
          ...
          case 'load_session':
            const loadResult = await this.toolHandlers.handleLoadSession(args as any);
            return {
              content: [
                {
                  type: 'text',
          ...
          case 'list_sessions':
            const listResult = await this.toolHandlers.handleListSessions(args as any);
            return {
              content: [
                {
                  type: 'text',
          ...
          case 'resume_session':
            const resumeResult = await this.toolHandlers.handleResumeSession(args as any);
            return {
              content: [
                {
                  type: 'text',
          ...
          case 'archive_session':
            const archiveResult = await this.toolHandlers.handleArchiveSession(args as any);
            return {
              content: [
                {
                  type: 'text',
          ...
          case 'validate_session_health':
            const healthResult = await this.toolHandlers.handleValidateSessionHealth(args as any);
            return {
              content: [
                {
                  type: 'text',
          ...
          case 'recover_session':
            const recoveryResult = await this.toolHandlers.handleRecoverSession(args as any);
            return {
              content: [
                {
                  type: 'text',
          ...
          case 'get_session_events':
            const eventsResult = await this.toolHandlers.handleGetSessionEvents(args as any);
            return {
              content: [
                {
                  type: 'text',
          ...
          case 'subscribe_to_events':
            const subscribeResult = await this.toolHandlers.handleSubscribeToEvents(args as any);
            return {
              content: [
                {
                  type: 'text',
          ...
          case 'unsubscribe_from_events':
            const unsubscribeResult = await this.toolHandlers.handleUnsubscribeFromEvents(args as any);
            return {
              content: [
                {
                  type: 'text',
          ...
          case 'resolve_conflicts':
            const resolveConflictsResult = await this.toolHandlers.handleResolveConflicts(args as any);
            return {
              content: [
                {
                  type: 'text',
          ...
          case 'update_state_with_conflict_resolution':
            const updateStateResult = await this.toolHandlers.handleUpdateStateWithConflictResolution(args as any);
            return {
              content: [
                {
                  type: 'text',
        ...
        switch (uri) {
          ...
          case 'thinking://session_history':
            ...
            const trends = {
              ...
              strategiesUsed: [...new Set(sessions.map((s: any) => s.strategy))],
              averageSteps: sessions.length > 0 ? sessions.reduce((sum: number, s: any) => sum + s.totalSteps, 0) / sessions.length : 0,
              statusDistribution: sessions.reduce((acc: any, s: any) => {
                acc[s.status] = (acc[s.status] || 0) + 1;
                return acc;
              }, {}),
              recentActivity: sessions.slice(0, 10).map((s: any) => ({
                id: s.id,
      ...
      try {
        switch (name) {
          ...
          case 'generate_solution_strategy':
            ...
            const strategyPreference = args?.strategy_preference as string;
          ...
          case 'validate_reasoning_chain':
            const reasoningState = args?.reasoning_state as string;
          ...
          case 'analyze_session_patterns':
            const sessionIds = Array.isArray(args?.session_ids) ? args.session_ids as string[] : undefined;
          ...
          case 'compare_sessions':
            const sessionAId = args?.session_a_id as string;
            const sessionBId = args?.session_b_id as string;
            const comparisonAspects = args?.comparison_aspects as string;
