...
export interface RecoveryMetrics {
  ...
  successRate: number; // Historical success rate for this strategy
}
...
export class SessionRecovery {
  ...
  private recoveryStats: Map<string, RecoveryMetrics>;
      ...
      for (let attempt = 1; attempt <= this.config.maxRetryAttempts; attempt++) {
        ...
        try {
          ...
          const result = await this.executeWithTimeout(
            strategy.execute(recoveryContext),
  ...
  private async executeFullStateRecovery(context: RecoveryContext): Promise<RecoveryResult> {
    const startTime = Date.now();
  ...
  private async executeCheckpointRollback(context: RecoveryContext): Promise<RecoveryResult> {
    const startTime = Date.now();
    ...
    try {
      ...
      const recoveredState: ReasoningState = {
  ...
  private async executePartialReconstruction(context: RecoveryContext): Promise<RecoveryResult> {
    const startTime = Date.now();
  ...
  private async executeMinimalRecovery(context: RecoveryContext): Promise<RecoveryResult> {
    const startTime = Date.now();
    
    try {
      // Create a minimal working state
      const minimalState: ReasoningState = {
  ...
  private createEmptyMetrics(recoveryTimeMs: number): RecoveryMetrics {
    return {
      recoveryTimeMs,
  ...
  private updateStrategyMetrics(strategyName: string, metrics: RecoveryMetrics, success: boolean): void {
