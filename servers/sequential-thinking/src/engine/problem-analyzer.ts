...
export class ProblemAnalyzer {
  ...
  async analyzeProblem(
    description: string, 
    initialContext?: Partial<ProblemContext>
    ...
    if (initialContext) {
      ...
      if (initialContext.domain) problem.domain = initialContext.domain;
   ...
   */
  async recommendStrategy(problem: ProblemDefinition): Promise<DecompositionStrategy> {
   ...
   */
  async getDetailedAnalysis(problem: ProblemDefinition) {
    const patterns = identifyProblemPatterns(problem);
    const strategy = selectDecompositionStrategy(problem, patterns);
   ...
   */
  private extractContextFromDescription(description: string): Record<string, unknown> {
   ...
   */
  private extractGoalState(description: string): string {
    const text = description.toLowerCase();
   ...
   */
  private assessComplexity(description: string): 'simple' | 'moderate' | 'complex' | 'expert' {
    const text = description.toLowerCase();
    ...
    else if (words.length > 50) complexityScore += 1;
    ...
    const domainCount = domains.filter(domain => 
      text.includes(domain) || this.getDomainKeywords(domain).some(keyword => text.includes(keyword))
    ).length;
