...
export class ValidationEngine {
   ...
   */
  async assessQuality(state: ReasoningState): Promise<SolutionQuality> {
   ...
   */
  async validateReasoningChain(state: ReasoningState): Promise<ValidationResult> {
   ...
   */
  async validateStep(step: ReasoningStep, context?: ReasoningState): Promise<{
    isValid: boolean;
    issues: ValidationIssue[];
    suggestions: string[];
  }> {
  ...
  private async assessCompleteness(state: ReasoningState): Promise<number> {
    ...
    const hasAnalysis = steps.some(s => 
      s.description.toLowerCase().includes('analyz') || 
      s.description.toLowerCase().includes('understand') ||
      s.description.toLowerCase().includes('research')
    );
    if (hasAnalysis) completenessScore += 0.15;

    // Implementation phase
    const hasImplementation = steps.some(s => 
      s.description.toLowerCase().includes('implement') || 
      s.description.toLowerCase().includes('create') ||
      s.description.toLowerCase().includes('build')
    );
    if (hasImplementation) completenessScore += 0.15;
    ...
    if (hasValidation) completenessScore += 0.1;

    // Constraint coverage
    const constraintsCovered = problem.constraints.filter(constraint => 
      steps.some(s => s.description.toLowerCase().includes(constraint.toLowerCase()))
    ).length;
  ...
  private async assessFeasibility(state: ReasoningState): Promise<number> {
    let feasibilityScore = 0.8; // Start optimistic

    const steps = state.steps;

    // Check for overly complex or vague steps
    const complexSteps = steps.filter(s => 
      s.inputs?.complexity === 'high' || 
      s.confidence < 0.5
    ).length;
  ...
  private async assessEfficiency(state: ReasoningState): Promise<number> {
    ...
    const parallelSteps = steps.filter(s => 
      ...
      steps.filter(other => other.dependencies.includes(s.id)).length === 0
    ).length;
  ...
  private async assessRobustness(state: ReasoningState): Promise<number> {
    ...
    if (hasErrorHandling) robustnessScore += 0.15;

    // Check for validation steps
    const validationSteps = steps.filter(s => 
      s.inputs?.category === 'validation' ||
      s.description.toLowerCase().includes('verify') ||
    ...
    const hasCheckpoints = steps.some(s => 
      s.description.toLowerCase().includes('checkpoint') || 
      s.description.toLowerCase().includes('save') ||
      s.description.toLowerCase().includes('backup')
    );
    if (hasCheckpoints) robustnessScore += 0.1;
  ...
  private async assessInnovation(state: ReasoningState): Promise<number> {
    ...
    const approachTypes = new Set(steps.map(s => s.inputs?.category as string));
    ...
    const conventionalSteps = steps.filter(s => 
      conventionalKeywords.some(keyword => s.description.toLowerCase().includes(keyword))
    ).length;
  ...
  private async assessConfidence(state: ReasoningState): Promise<number> {
    const steps = state.steps;
    const avgConfidence = steps.reduce((sum, s) => sum + s.confidence, 0) / steps.length;
  ...
  private async validateLogicalConsistency(state: ReasoningState): Promise<ValidationIssue[]> {
    const issues: ValidationIssue[] = [];
    const steps = state.steps;
  ...
  private async validateFeasibility(state: ReasoningState): Promise<ValidationIssue[]> {
    const issues: ValidationIssue[] = [];
    const steps = state.steps;
  ...
  private async validateConstraintCompliance(state: ReasoningState): Promise<ValidationIssue[]> {
    const issues: ValidationIssue[] = [];
    ...
    for (const constraint of constraints) {
      ...
      switch (constraint) {
        ...
        case 'risk_averse':
          addressed = steps.some(s => 
            s.description.toLowerCase().includes('safe') ||
            s.description.toLowerCase().includes('backup') ||
  ...
  private async validateDependencies(state: ReasoningState): Promise<ValidationIssue[]> {
    const issues: ValidationIssue[] = [];
    const steps = state.steps;
    ...
    for (const step of steps) {
      ...
      const laterDeps = step.dependencies.filter(depId => 
        laterSteps.some(s => s.id === depId)
      );
  ...
  private async validateStepQuality(state: ReasoningState): Promise<ValidationIssue[]> {
    const issues: ValidationIssue[] = [];
    const steps = state.steps;
  ...
  private isStepActionable(step: ReasoningStep): boolean {
  ...
  private validateStepInContext(step: ReasoningStep, context: ReasoningState): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
  ...
  private checkLogicalConnection(step: ReasoningStep, previousSteps: ReasoningStep[]): boolean {
    ...
    const commonWords = stepWords.filter(word => 
      word.length > 3 && previousWords.includes(word)
    );

    return commonWords.length >= 2; // At least 2 words in common
  }

  private detectCircularDependencies(steps: ReasoningStep[]): string[][] {
    const cycles: string[][] = [];
  ...
  private identifyRedundantSteps(steps: ReasoningStep[]): ReasoningStep[] {
    const redundant: ReasoningStep[] = [];
  ...
  private calculateStepSimilarity(step1: ReasoningStep, step2: ReasoningStep): number {
  ...
  private checkGoalAlignment(step: ReasoningStep, problem: ProblemDefinition): boolean {
    const stepWords = step.description.toLowerCase().split(/\s+/);
    const goalWords = problem.goalState.toLowerCase().split(/\s+/);
    
    const commonWords = stepWords.filter(word => 
      word.length > 3 && goalWords.includes(word)
    );

    return commonWords.length >= 1; // At least one meaningful word in common
  }

  private generateRecommendations(state: ReasoningState, issues: ValidationIssue[]): string[] {
    const recommendations: string[] = [];
