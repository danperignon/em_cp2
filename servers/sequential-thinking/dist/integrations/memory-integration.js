...
export class MemoryIntegration {
    ...
    namespace;
    isAvailable = false;
    constructor(namespace = 'sequential-thinking') {
     ...
     */
    async storeReasoningPattern(pattern, options = {}) {
        if (!this.isAvailable) {
     ...
     */
    async getReasoningPatterns(problemType, domain, complexity) {
        if (!this.isAvailable) {
            return [];
        }
        try {
            ...
            const patterns = await this.queryMemory(searchKey);
     ...
     */
    async storeSolution(solution, tags = [], rating = 3) {
        if (!this.isAvailable) {
            return 'memory-unavailable';
        }
        try {
     ...
     */
    async findSimilarSolutions(problemFingerprint, limit = 5) {
        if (!this.isAvailable) {
            return [];
        }
        try {
            const searchKey = `${this.namespace}/solutions/${problemFingerprint}/*`;
            const solutions = await this.queryMemory(searchKey);
     ...
     */
    async updatePatternPerformance(patternId, quality, duration) {
        if (!this.isAvailable)
            return;
        try {
            ...
            const avgQuality = pattern.qualityScores.reduce((sum, q) => sum + this.calculateOverallQuality(q), 0) / pattern.qualityScores.length;
     ...
     */
    async getCognitiveInsights(problemDefinition) {
        if (!this.isAvailable) {
            return {
        ...
        try {
            ...
            const patterns = await this.getReasoningPatterns(this.classifyProblemType(problemDefinition), problemDefinition.domain, problemDefinition.complexity);
    ...
    async storeInMemory(key, _data, _options = {}) {
    ...
    async queryMemory(searchPattern) {
    ...
    calculateOverallQuality(quality) {
        ...
        return (quality.completeness * 0.25 +
    ...
    classifyProblemType(problem) {
        ...
        if (description.includes('analyze') || description.includes('evaluate'))
            return 'analytical';
        if (description.includes('create') || description.includes('design'))
            return 'creative';
        if (description.includes('fix') || description.includes('debug'))
            return 'diagnostic';
        if (description.includes('plan') || description.includes('strategy'))
            return 'planning';
        if (description.includes('research') || description.includes('investigate'))
            return 'research';
        if (description.includes('optimize') || description.includes('improve'))
            return 'optimization';
