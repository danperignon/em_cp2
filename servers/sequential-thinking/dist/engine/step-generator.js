...
export class StepGenerator {
     ...
     */
    async createStepSequence(stepDescriptions, baseOptions = {}) {
        const steps = [];
     ...
     */
    async createParallelSteps(stepDescriptions, sharedDependencies = [], baseOptions = {}) {
        const steps = [];
        for (let i = 0; i < stepDescriptions.length; i++) {
            const step = await this.createStep({
                description: stepDescriptions[i],
     ...
     */
    enhanceReasoning(reasoning, description) {
     ...
     */
    calculateDefaultConfidence(options) {
     ...
     */
    calculateSequenceConfidence(stepIndex, totalSteps) {
     ...
     */
    estimateStepDuration(description) {
     ...
     */
    assessStepComplexity(description) {
        const desc = description.toLowerCase();
    ...
    categorizeStep(description) {
        ...
        if (desc.includes('test') || desc.includes('validate') || desc.includes('verify'))
            return 'validation';
        if (desc.includes('setup') || desc.includes('configure'))
            return 'setup';
        if (desc.includes('integrate') || desc.includes('combine'))
            return 'integration';
        return 'general';
    }
}
