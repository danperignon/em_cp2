...
export class ProgressiveRestoration {
    ...
    currentPlan;
    activeRestorations;
    constructor(stateStorage, sessionValidator, sessionRecovery, eventManager, config = {}) {
     ...
     */
    async calculatePriorities(sessions) {
        const priorities = [];
     ...
     */
    createRestorationStages(priorities) {
        const stages = [];
     ...
     */
    async executeRestorationStage(stage, report) {
    ...
    async restoreSingleSession(sessionId) {
        ...
        try {
            ...
            const healthReport = await this.sessionValidator.validateSession(loadResult.state, loadResult.metadata);
            ...
            if (healthReport.healthScore < this.config.healthScoreThreshold) {
                strategy = 'recovery';
                const recoveryResult = await this.sessionRecovery.recoverSession(sessionId, 'corruption', {
    ...
    calculateDependencyScore(_session) {
    ...
    calculateTotalPriorityScore(factors) {
        ...
        return Math.round(factors.lastActivity * weights.lastActivity +
    ...
    estimateRestorationTime(stages) {
    ...
    async performQuickHealthCheck(sessionId) {
        try {
    ...
    createErrorReport(error, startTime) {
        return {
            success: false,
            totalSessions: 0,
            successfulRestorations: 0,
...
//# sourceMappingURL=progressive-restoration.js.map