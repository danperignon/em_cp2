...
export class DecompositionEngine {
     ...
     */
    async topDownDecomposition(problem, options) {
        const steps = [];
     ...
     */
    async bottomUpDecomposition(problem, options) {
        const steps = [];
     ...
     */
    async divideConquerDecomposition(problem, _options) {
        const steps = [];
     ...
     */
    async incrementalDecomposition(_problem, options) {
        const steps = [];
     ...
     */
    async parallelDecomposition(problem, _options) {
        const steps = [];
     ...
     */
    async iterativeDecomposition(_problem, options) {
        const steps = [];
        // Initial prototype
    ...
    identifyAtomicActions(_problem) {
    ...
    generateIncrementalSteps(_problem, maxSteps) {
        const baseActions = [
    ...
    identifyParallelWorkstreams(_problem) {
        return [
    ...
    identifySynchronizationPoints(_workstreams) {
        return [
            'Integration checkpoint',
            'End-to-end testing synchronization'
        ];
    }
    async createDetailedTasks(subGoals, existingSteps, remainingDepth) {
        if (remainingDepth <= 0)
            return [];
        const detailSteps = [];
    ...
    async groupIntoComponents(_atomicActions, atomicSteps) {
    ...
    async postProcessSteps(steps, options) {
...
//# sourceMappingURL=decomposition-engine.js.map