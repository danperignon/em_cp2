...
export class IntegrationTestScenarios {
     ...
     */
    async testCognitiveWorkspaceDemo() {
     ...
     */
    async testMemoryEnhancedSolving() {
        const startTime = Date.now();
    ...
    async testArtifactGeneration() {
      ...
      `;
            ...
            const actualTypes = [...new Set(manifest.artifacts.map((a) => a.type))];
            ...
            const hasOverview = manifest.artifacts.some((a) => a.filename.includes('overview'));
            const hasStepGuide = manifest.artifacts.some((a) => a.filename.includes('step-by-step'));
            const hasTemplate = manifest.artifacts.some((a) => a.filename.includes('template'));
            ...
            return {
                ...
                metrics: {
                    ...
                    averageArtifactSize: manifest.artifacts.reduce((sum, a) => sum + a.size, 0) / manifest.artifacts.length
                }
            };
        }
        catch (error) {
     ...
     */
    async testMultiStrategyLearning() {
        const startTime = Date.now();
        ...
        try {
            ...
            const bestStrategy = results.reduce((best, current) => current.confidence > best.confidence ? current : best);
            return {
                scenario,
                success: true,
                duration: Date.now() - startTime,
     ...
     */
    async testSoftwareDevelopmentScenario() {
        const startTime = Date.now();
      ...
      `;
            ...
            const missingComponents = requiredComponents.filter(comp => !stepDescriptions.includes(comp));
     ...
     */
    async testResearchAnalysisIntegration() {
        const startTime = Date.now();
      ...
      `;
            ...
            const coveredElements = researchElements.filter(element => stepDescriptions.includes(element));
            ...
            const hasAnalysisTemplate = artifactManifest.artifacts.some((a) => a.filename.includes('template') || a.description.toLowerCase().includes('analysis'));
            return {
                scenario,
                success: true,
                duration: Date.now() - startTime,
...
//# sourceMappingURL=integration-scenarios.js.map