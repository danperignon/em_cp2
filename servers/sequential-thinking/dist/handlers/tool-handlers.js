...
export class ToolHandlers {
    ...
    async handleDecomposeProblem(args) {
        try {
            ...
            const problemDefinition = await this.problemAnalyzer.analyzeProblem(args.problem_description, {
                domain: 'general',
                constraints: [],
            });
            ...
            const reasoningSteps = await this.decompositionEngine.decompose(problemDefinition, strategy, {
                strategy,
            ...
            const reasoningState = await this.chainManager.createReasoningState(problemDefinition, reasoningSteps, strategy);
     ...
     */
    validateDecomposeProblemInput(args) {
     ...
     */
    classifyProblemType(description) {
        const text = description.toLowerCase();
        if (text.includes('analyze') || text.includes('evaluate'))
            return 'analytical';
        if (text.includes('create') || text.includes('design'))
            return 'creative';
     ...
     */
    generateProblemFingerprint(problem) {
        const parts = [
    ...
    async handleResumeSession(args) {
        try {
            ...
            const resumeResult = await this.chainManager.resumeSession(args.session_id, args.continue_from_step);
    ...
    async handleValidateSessionHealth(args) {
        try {
            ...
            const result = await this.chainManager.validateSessionHealth(args.session_id, args.attempt_auto_repair || false);
            if (result.success) {
                return {
                    success: true,
    ...
    async handleRecoverSession(args) {
        try {
            ...
            const result = await this.chainManager.recoverSession(args.session_id, args.error_type || 'unknown');
            if (result.success) {
                return {
                    success: true,
     ...
     */
    async handleGetRestorationMetrics() {
        try {
    ...
    async handleSubscribeToEvents(args) {
        try {
            ...
            const subscriptionId = this.chainManager.subscribeToEvents(args.event_types, handler, {
    ...
    async handleUpdateStateWithConflictResolution(args) {
        try {
            ...
            const result = await this.chainManager.updateStateWithConflictResolution(args.session_id, args.client_id, args.state_update);
            return {
                success: result.success,
